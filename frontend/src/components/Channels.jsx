...
export default class Channels extends React.Component {
  ...
  async fetchArticles() {
    await this.setState({
      url: 'http://localhost:8000/api/pipeline/?ticker=' + this.state.ticker,
    });
  ...
  render() {
    ...
    const listData = pipeline.map((pipelines) => (
      <div className='listboxtext listbox'>
        <a href='https://localhost:3000/' className='listboxevent'>
          {pipelines.event}
        </a>
        <a
          href={
            this.thumbnailNeeded(pipelines.source_youtube)
              ? 'https://localhost:3000/'
              : pipelines.source_youtube
          }
          className={
            this.thumbnailNeeded(pipelines.source_youtube)
              ? 'listboxgeneral'
              : 'listboxgeneral youtube'
          }
        >
          {pipelines.source_youtube === '+' && '+'}
        </a>
        <a
          href={
            this.thumbnailNeeded(pipelines.source_twitter)
              ? 'https://localhost:3000/'
              : pipelines.source_twitter
          }
          className={
            this.thumbnailNeeded(pipelines.source_twitter)
              ? 'listboxgeneral'
              : 'listboxgeneral twitter'
          }
        >
          {pipelines.source_twitter === '+' && '+'}
        </a>
        <a
          href={
            this.thumbnailNeeded(pipelines.source_other)
              ? 'https://localhost:3000/'
              : pipelines.source_other
          }
          className={
            this.thumbnailNeeded(pipelines.source_other)
              ? 'listboxgeneral'
              : 'listboxgeneral other'
          }
        >
          {pipelines.source_other === '+' && '+'}
        </a>
        <a href='https://localhost:3000/' className='listboxgeneral'>
          {pipelines.date_of_event}
        </a>
        <a href='https://localhost:3000/' className='listboxgeneral'>
          {pipelines.date_added}
        </a>
        <a href='https://localhost:3000/' className='listboxgeneral'>
          {pipelines.primary_category}
        </a>
        <a href='https://localhost:3000/' className='listboxgeneral'>
          {pipelines.secondary_category}
        </a>
        <a href='https://localhost:3000/' className='listboxgeneral'>
          {pipelines.significance}
        </a>
      </div>
    ));
    return (
